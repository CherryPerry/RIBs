...
open class Node<V : RibView>(
    ...
    open fun onDestroy(recreateSoon: Boolean) {
        if (view != null) {
            RIBs.errorHandler.handleNonFatalError(
        ...
        if (!recreateSoon) {
            Store.removeAll(identifier)
        }

        for (child in children.toList()) {
            detachChildNode(child, recreateSoon)
        }
